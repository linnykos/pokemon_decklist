decklist <- function(deck_file, filename){
  ...
  for(i in 1:n){
    ...
    graphics::rasterImage(img_cropped, 149, (n-i)*th+35, 149+w, (n-i)*th+35+h)

    TeachingDemos::shadowtext(148, (n-i)*th+35+h/2, txt_mat[i,2],
                              pos = 4, cex = 2, r = 0.3)

    if(txt_mat[i,1] != 1){
      TeachingDemos::shadowtext(875, (n-i)*th+20+h/2, txt_mat[i,1], col = "gold",
                                pos = 2, cex = 3, r = 0.3)
    }
  }
